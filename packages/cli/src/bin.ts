...
import { ChatRendererSchema, detectRenderer, initChatProject } from './index.js'

declare const __CLI_VERSION__: string

const help = `Usage: agentskit-chat init [directory] [--renderer react|react-native|ink] [--yes]
...
const completion = (shell: string | undefined): string => {
  if (shell === 'bash') return 'complete -W "init completion --help --version" agentskit-chat\n'
  if (shell === 'zsh') return '#compdef agentskit-chat\n_arguments "1:command:(init completion)" "--renderer[renderer]:renderer:(react react-native ink)"\n'
  if (shell === 'fish') return 'complete -c agentskit-chat -f -a "init completion"\ncomplete -c agentskit-chat -l renderer -a "react react-native ink"\n'
  throw new Error(`Expected completion shell bash, zsh, or fish.\n${help}`)
}

const main = async (): Promise<void> => {
  const parsed = parseArgs({ allowPositionals: true, strict: true, options: { renderer: { type: 'string' }, yes: { type: 'boolean', short: 'y' }, help: { type: 'boolean', short: 'h' }, version: { type: 'boolean', short: 'v' } } })
  ...
  if (!renderer && !parsed.values.yes && stdin.isTTY && stdout.isTTY) {
    ...
    try { renderer = await prompt.question('Renderer (react, react-native, ink): ') } finally { prompt.close() }
  }
