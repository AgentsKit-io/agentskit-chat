...
import { SCENES, type Scene, type WidgetKind } from './scenes'
import { ApprovalCard, FlightList, MobileShell, TerminalMirror } from './widgets'
...
function Dot({ delay = 0 }: { delay?: number }) {
  return (
    <span
      className="inline-block h-1.5 w-1.5 animate-pulse rounded-full bg-ak-graphite"
...
function Spinner() {
  return (
    <span className="inline-block h-3 w-3 animate-spin rounded-full border border-ak-blue/30 border-t-ak-blue" />
  )
}
...
export function HeroDemo() {
  ...
  useEffect(() => {
    ...
    const applyEvent = async (ev: Scene['events'][number]) => {
      if (cancelled) return
      switch (ev.type) {
        case 'userType': {
          const cps = ev.cps ?? 34
        ...
        case 'thinking':
          patch({ thinking: true })
          await sleep(reducedMotion ? 100 : 480)
          return
        case 'tool':
        ...
        case 'widget':
          ...
          await sleep(reducedMotion ? 80 : 320)
          return
    ...
    const run = async () => {
      ...
      await sleep(200)
      if (!cancelled) setSceneIdx((i) => (i + 1) % SCENES.length)
    }
    void run()
  ...
  return (
    <div className="flex min-w-0 flex-col overflow-hidden rounded-xl border border-ak-border bg-ak-surface shadow-2xl shadow-black/40">
      <div className="flex items-center justify-between border-b border-ak-border px-4 py-2.5">
        ...
        <span className="font-mono text-xs text-ak-graphite">chat.agentskit.io · {scene.surface}</span>
            ...
            <div className="flex w-full min-w-0 justify-start">
              <div className="flex w-full min-w-0 max-w-[92%] flex-col gap-2">
                ...
                {frame.tool ? (
                  ...
                  >
                    <span className="mt-0.5 shrink-0">{frame.tool.done ? '✓' : <Spinner />}</span>
                    <span className="min-w-0 break-all">{frame.tool.label}</span>
                    {frame.tool.done ? <span className="shrink-0 text-ak-graphite">{frame.tool.ms}ms</span> : null}
                  </div>
                ) : null}

                {frame.widget ? (
                  <div key={frame.widget} className="min-w-0 max-w-full">
                    {frame.widget === 'flight' ? <FlightList /> : null}
                    {frame.widget === 'approval' ? <ApprovalCard /> : null}
                    {frame.widget === 'terminal' ? <TerminalMirror /> : null}
                    {frame.widget === 'mobile' ? <MobileShell /> : null}
                  </div>
                ) : null}

                {frame.assistant ? (
                  <div className="w-fit max-w-full rounded-2xl rounded-bl-md bg-ak-surface px-3.5 py-2 text-left text-ak-foam">
                    {frame.assistant}
        ...
        <div className="flex min-w-0 items-center gap-2 border-t border-ak-border bg-ak-surface px-3 py-2.5">
          ...
          <span className="min-w-0 flex-1 truncate text-ak-foam">
            ...
            {frame.userDraft ? (
              ...
              <span className="text-ak-graphite">ask anything…</span>
            )}
          </span>
      ...
      <div className="flex items-center gap-1 border-t border-ak-border bg-ak-surface px-2 py-2">
        ...
        >
          {SCENES.map((s, i) => (
            ...
            >
              <span className={i === sceneIdx ? 'mr-1 inline-block h-1.5 w-1.5 rounded-full bg-ak-blue' : ''} />
              {s.label}
            </button>
          ))}
        </div>
