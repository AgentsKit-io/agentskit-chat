...
const main = async () => {
  ...
  await wait(500)
  await setCaption('Four known prompts: local answers, no guessing, no model.')
  await wait(1_600)
  await setCaption('1 / 4  ·  hi → deterministic answer, no model call.')
  await typeAndSend('hi', 2_500)
  await setCaption('2 / 4  ·  how can I call you? → prepared, instant answer.')
  await typeAndSend('how can I call you?', 2_500)
  await setCaption('3 / 4  ·  what day is today → local custom component.')
  await typeAndSend('what day is today', 3_000)
  await setCaption('4 / 4  ·  toggle the mode → local interface action.')
  await typeAndSend('toggle the mode', 2_500)
  await setCaption('Once more → switching back to dark mode.')
  await typeAndSend('toggle the mode', 2_500)
  await setCaption('Surprise me → no local rule: escalating to AI automatically.')
  await typeAndSend('Surprise me', 3_000)
  await setCaption('Now, AI mode: every prompt goes straight to the model.')
  await clickWithIndicator(demo.getByRole('button', { name: 'AI', exact: true }))
  await wait(1_200)
  await setCaption('Same “hi”, different path: the answer now comes from AI.')
  await typeAndSend('hi', 2_800)
  await setCaption('AI 2 / 4  ·  how can I call you? → straight to the model.')
  await typeAndSend('how can I call you?', 3_500)
  await setCaption('AI 3 / 4  ·  toggle the mode → Markdown rendered in the answer.')
  await typeAndSend('toggle the mode', 4_000)
  await setCaption('AI 4 / 4  ·  what day is today → even known questions call the model.')
  await typeAndSend('what day is today', 3_500)
  await setCaption('Use AI where you need it; be deterministic where you know the answer.')
