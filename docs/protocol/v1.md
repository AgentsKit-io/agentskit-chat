...
Every turn event may carry an optional `correlation` object. Its required
`operationId` is the only identity shared across AgentsKit boundaries; the
optional `runId`, `sessionId`, `turnId`, `actionId`, and `traceId` fields keep
their owner-specific meaning. Producers preserve the object, consumers ignore
it when absent, and secrets or payload content must never be placed in it.
