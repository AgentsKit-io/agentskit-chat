...
`packages/svelte` owns only the Svelte 5 application shell, typed snippets, semantic CSS mapping, and native `ChoiceList`. It consumes `createChatStore`, `ChatContainer`, `Message`, `InputBar`, `ThinkingIndicator`, and `ToolConfirmation` from `@agentskit/svelte@0.4.3`.
