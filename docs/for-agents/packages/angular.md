...
`packages/angular` owns only the standalone Angular application shell, content-template composition, semantic CSS mapping, specialized `ChoiceList`, and generic native `StandardComponentComponent`. It consumes `AgentskitChat`, `ChatContainerComponent`, `MessageComponent`, `InputBarComponent`, `ThinkingIndicatorComponent`, and `ToolConfirmationComponent` from the typed partial-Ivy `@agentskit/angular@0.4.6` package.
