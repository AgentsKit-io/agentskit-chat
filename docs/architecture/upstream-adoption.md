...
AgentsKit Chat Protocol adds only versioned application schemas for site configuration, immutable local knowledge, answer provenance, citations, suggestions, confidence, and escalation. AgentsKit Chat adds only bounded normalized exact lookup, projection into its existing ordered assistant-content and standard component protocols, and delegation to the injected upstream adapter. Artifact generation/cache/hashing remain host or Doc Bridge concerns. No controller, reducer, adapter lifecycle, retrieval engine, statechart, renderer primitive, upstream source, private source, or business data is copied or reimplemented. See proposed [ADR-0024](./adrs/0024-deterministic-answer-plane.md).
