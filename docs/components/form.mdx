...
| Event | Value | Meaning |
| --- | --- | --- |
| `submit` | `form` | Record of field id → value |
  ...
  return (
    <AgentChat
      ...
      onComponentInteract={(event) => {
        // { type: 'submit', componentKey: 'form', instanceId, value: { email: string, ... } }
      }}
    />
  )
}
...
| Event shape | `{ type: 'submit', componentKey: 'form', instanceId, value: { email: string, ... } }` |
